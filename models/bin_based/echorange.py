...
class EchoRangeDepth(nn.Module):
    ...
    def __init__(self,
                 ...
                 range_output_quantile: float = 0.5,
                 range_output_temperature: float = 1.0,
                 range_bin_axis: str = "radial",
        ...
        # The bin axis is decided here so checkpoints embed it; the train
        # step reads the same field to convert GT depth into the matching
        # axis (radial → ρ_xy or z) before computing per-bin losses.
        if range_bin_axis not in ("radial", "horizontal", "z"):
            raise ValueError(
                "range_bin_axis must be one of 'radial', 'horizontal', 'z'; "
                f"got {range_bin_axis!r}")
        self.range_bin_axis = str(range_bin_axis)
        ...
        if self.depth_head_type == "scalar":
            ...
            self.range_head = RangeDepthHead(
                ...
                output_quantile=float(range_output_quantile),
                output_temperature=float(range_output_temperature),
            )
        else:  # 'hazard'
            self.scalar_head = None
